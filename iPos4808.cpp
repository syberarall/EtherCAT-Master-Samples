...
void static DoLogic(PDRIVE_INFO pDriveInfo)
{
	...
	pDriveInfo->RxPdoMap.TargetPos += 200;
...
{
	//Enable homing
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ModeOP = 6; }
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord  = 0x80; } Sleep(100);
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord |= 0x06; } Sleep(100);
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord |= 0x07; } Sleep(100);
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord |= 0x0F; } Sleep(100);
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord |= 0x1F; } Sleep(100);

	//Startup drive
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ModeOP = 8; }
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord  = 0x80; } Sleep(100);
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord |= 0x06; } Sleep(100);
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord |= 0x07; } Sleep(100);
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord |= 0x0F; } Sleep(100);
...
__inline void __DisableDrives(void)
{
	...
	for (int i=0; i<__DriveNum; i++) { __DriveList[i].RxPdoMap.ControlWord = 0x80; }
						...
						if (ERROR_SUCCESS == Ecat64CyclicStateControl(&__StateObject, AL_STATE_OP))      { Sleep(100);
								...
								while (!kbhit())
								{
									...
									printf("Update:%i, S0:0x%04x P0:0x%08x, S1:0x%04x P1:0x%08x, S2:0x%04x P2:0x%08x, S3:0x%04x P3:0x%08x\r",
										...
										__DriveList[0].TxPdoMap.ActPos,
										__DriveList[1].TxPdoMap.StatusWord,
										__DriveList[1].TxPdoMap.ActPos,
										__DriveList[2].TxPdoMap.StatusWord,
										__DriveList[2].TxPdoMap.ActPos,
										__DriveList[3].TxPdoMap.StatusWord,
										__DriveList[3].TxPdoMap.ActPos);
