...
ULONG			__EcatState = 0;		//CombinedLogic Wrapper State
...
/*
		//*********************************
		//e.g. EK1100, EL4132, EL1018, EL2008
		//Get logical image pointer of LRW telegram (only 1 global telegram)
		PUCHAR pImageRX = &__pSystemList[0].RxTel.s.data[0];
		PUCHAR pImageTX = &__pSystemList[0].TxTel.s.data[0];

		//Get data offsets within logical image
		ULONG EL1018_Offs = __pSystemList[2].ExtInfo.LogicalOffs;
		ULONG EL2008_Offs = __pSystemList[3].ExtInfo.LogicalOffs;

		//Do the logical station operation
		__InputData = pImageRX[EL1018_Offs];
		pImageTX[EL2008_Offs] = __OutputData;
		//*********************************
*/
		__UpdateCnt++;
	}
...
//******************
	...
	if (ERROR_SUCCESS == Sha64EcatCreate(&EcatParams))
	{
		...
		if (ERROR_SUCCESS == Ecat64ChangeAllStates(AL_STATE_INIT))
		{
			...
			if (ERROR_SUCCESS == Ecat64InitFmmusCombined(EcatParams.LogicalAddr))
			if (ERROR_SUCCESS == Ecat64InitSyncManagers())
			{
				...
				if (ERROR_SUCCESS == Ecat64PdoAssignment())
				{
...
#endif
					{
						...
						Ecat64InitProcessTelegramsCombined(EcatParams.LogicalAddr);
						...
						if (ERROR_SUCCESS == Ecat64CyclicStateControl(&__StateObject, AL_STATE_OP))		 { Sleep(100);
								...
								while (!kbhit())
								{
									...
									printf("Input: %02x, Loop Count: %i, Update Count: %i\r", __InputData, __LoopCnt, __UpdateCnt);
