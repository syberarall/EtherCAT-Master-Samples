...
#define UPDATES_PER_SEC		((1000 * 1000) / (REALTIME_PERIOD * SYNC_CYCLES))
...
void static AppTask(PVOID)
{
	...
	if (__ReadyCnt == 1)
	{
		//*********************************
		//Get input value
		__DiVal = __pSystemList[0].RxTel.s.data[0];

		//Increment timer (based on realtime period)
		__TimerVal = ((++__TimerVal) % UPDATES_PER_SEC);

		//Increment output value
		if (__TimerVal == 0) { __DoVal++; }

		//Set output value
		__pSystemList[0].TxTel.s.data[0] = __DoVal;
		//*********************************
...
void main(void)
{
	...
	EcatParams.EthParams.period = 200;				//Set realtime period [µsec]
	...
	if (ERROR_SUCCESS == Sha64EcatCreate(&EcatParams))
	{
		...
		if (ERROR_SUCCESS == Sha64EcatEnable(&EcatParams))
		{
			...
			while (!kbhit())
			{
				...
				printf("Input: 0x%02x Loop Count: %i, Update Count: %i\r", __DiVal, __LoopCnt, __UpdateCnt);
