...
//******************
	...
	if (ERROR_SUCCESS == Sha64EcatCreate(&EcatParams))
	{
		...
		if (ERROR_SUCCESS == Ecat64ChangeAllStates(AL_STATE_INIT))
		{
			...
			//Get station index
			__DeviceIndex = __GetStationIndexByName(__pUserList, __StationNum, "EL3702");
				...
				if (ERROR_SUCCESS == Ecat64PdoAssignment())
				{
...
#endif
					{
						...
						if (ERROR_SUCCESS == Ecat64CyclicStateControl(&__StateObject, AL_STATE_OP))      { Sleep(100);
								...
								//Get station pointer
								PSTATION_INFO pDrive = &__pUserList[__DeviceIndex];
								...
								while (!kbhit())
								{
									...
									printf("Update Count: %i, CH1: 0x%04x [%04x-%04x-%04x ...] CH2: 0x%04x [%04x-%04x-%04x ...]\r",
										__UpdateCnt,
										__TxPdoMap.Ch1Count,
