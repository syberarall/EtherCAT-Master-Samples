...
/*
#define MAX_ENTRY_GAP	5
__inline void __AdjustRxTable(PETH_TABLE pRxTable)
{
	ULONG FIndex = __pSystemStack->rx_table.findex;
	ULONG BIndex = __pSystemStack->rx_table.bindex;
	PUCHAR pFFrameData = (PUCHAR)__pSystemStack->rx_table.list[FIndex].entry.pFrame->bytes;
	PUCHAR pBFrameData = (PUCHAR)__pSystemStack->rx_table.list[BIndex].entry.pFrame->bytes;
	ULONG FFrameLen = __pSystemStack->rx_table.list[FIndex].entry.FrameSize;
	ULONG BFrameLen = __pSystemStack->rx_table.list[BIndex].entry.FrameSize;
	PUSHORT pFWorkCnt = (PUSHORT)&pFFrameData[14 + pFFrameData[14]];
	PUSHORT pBWorkCnt = (PUSHORT)&pBFrameData[14 + pBFrameData[14]];
	BOOLEAN FOccupied = __pSystemStack->rx_table.list[FIndex].entry.bOccupied;
	BOOLEAN BOccupied = __pSystemStack->rx_table.list[BIndex].entry.bOccupied;

#ifdef SEQ_DEBUG
		SYSTEM_SEQ_CAPTURE("FIDX",__LINE__, __pSystemStack->rx_table.findex, TRUE);
		SYSTEM_SEQ_CAPTURE("BIDX",__LINE__, __pSystemStack->rx_table.bindex, FALSE);
		SYSTEM_SEQ_CAPTURE("FOCC",__LINE__, FOccupied, FALSE);
		SYSTEM_SEQ_CAPTURE("BOCC",__LINE__, BOccupied, FALSE);
		SYSTEM_SEQ_CAPTURE("FFL", __LINE__, FFrameLen, FALSE);
		SYSTEM_SEQ_CAPTURE("BFL", __LINE__, BFrameLen, FALSE);
		SYSTEM_SEQ_CAPTURE("FFD", __LINE__, *((PULONG64)&pFFrameData[12]), FALSE);
		SYSTEM_SEQ_CAPTURE("BFD", __LINE__, *((PULONG64)&pBFrameData[12]), FALSE);
		SYSTEM_SEQ_CAPTURE("BWC", __LINE__, (USHORT)*pFWorkCnt, FALSE);
		SYSTEM_SEQ_CAPTURE("FWC", __LINE__, (USHORT)*pBWorkCnt, FALSE);
#endif


	//Check for race condition
	if ((FIndex != BIndex) && (FOccupied == 0) && (BOccupied == 0))
	{
		//__pUserStack->hdr.run_flag = FALSE;
		//pRxTable->findex = BIndex;
	}

	if (((FIndex - BIndex) % MAX_ETH_ENTRIES) > (MAX_ETH_ENTRIES - MAX_ENTRY_GAP))
	{
		//__pUserStack->hdr.run_flag = FALSE;
		pRxTable->findex = BIndex;
	}
...
__inline void __SdoInitUploadReqWrite(
									...
									PUCHAR pSdoData)
{
	...
	if ((*pRxWorkCnt) == (__WorkCount + 1))
	{
		...
		ULONG MailboxOffs = sizeof(MBX_HDR) + sizeof(COE_HDR);

		PSDO_INIT_HDR pSdoInitHdr = (PSDO_INIT_HDR)&pMailbox[MailboxOffs];
		ULONG SdoSize = 0;

		//Increase mailbox offset
		MailboxOffs += sizeof(SDO_INIT_HDR);
		...
		if (pSdoInitHdr->s.bits.TransferType)
		{
			...
			{
			case 3 : SdoSize = 1; break;
			case 2 : SdoSize = 2; break;
			case 1 : SdoSize = 3; break;
			case 0 : SdoSize = 4; break;
			}
		}
		else
		{
			//Get data size out of data field and increase mailbox offset
			SdoSize = *(PULONG)&pMailbox[MailboxOffs];
			MailboxOffs += sizeof(ULONG);
		}
	
		//Return SDO data and size
		if (pSdoSize) { *pSdoSize = SdoSize; }
		if (pSdoData) { memcpy(pSdoData, &pMailbox[MailboxOffs], SdoSize); }
		return TRUE;
	}
...
/*
									...
									if (ERROR_SUCCESS != Ecat64SdoInitUploadReq( pStation, 0x8010, 0x01))
									{
										__pUserStack->hdr.run_flag = FALSE;
									}
