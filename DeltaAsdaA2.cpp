...
{
	//Set Position
	pDriveInfo->RxPdoMap.ControlWord = 0x001F;
...
__inline void __EnableDrives(void)
{
	...
	{
		//Startup drive
		__DriveList[i].RxPdoMap.TargetPos = __DriveList[i].TxPdoMap.ActualPos;
		__DriveList[i].RxPdoMap.ModeOfOp = 8;
...
__inline void __DisableDrives(void)
{
	...
	for (int i=0; i<__DriveNum; i++)
	{
		...
		__DriveList[i].RxPdoMap.ControlWord = 0x80;
		Sleep(100);
	}
